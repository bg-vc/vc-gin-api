...
func (sink *writerSink) Log(level LogLevel, log []byte) {
	...
	if sink.name == "stdout" {
		if bytes.Contains(log, []byte("WARN")) {
			log = bytes.Replace(log, []byte("WARN"), WarnByte, -1)
		} else if bytes.Contains(log, []byte("ERROR")) {
			log = bytes.Replace(log, []byte("ERROR"), ErrorByte, -1)
		} else if bytes.Contains(log, []byte("FATAL")) {
			log = bytes.Replace(log, []byte("FATAL"), FatalByte, -1)
		} else if bytes.Contains(log, []byte("DEBUG")) {
			log = bytes.Replace(log, []byte("DEBUG"), DebugByte, -1)
		}
	}
	log = append(log, '\n')
	sink.writeL.Lock()
